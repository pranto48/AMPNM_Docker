function initMap() {
    ...
    // Determine if the current user is a viewer
    ...
    if (!isViewer) { // Only attach form listeners for non-viewers
    ...
    } // End if (!isViewer) block for form listeners
    ...
    (async () => {
        ...
        if (initialMapId) {
            ...
            if (deviceToEdit && state.nodes.get(deviceToEdit) && !isViewer) { // Only open edit modal if not viewer
