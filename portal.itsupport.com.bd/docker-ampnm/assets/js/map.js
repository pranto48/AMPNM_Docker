...
function initMap() {
    ...
    // Note: Most device/edge form logic and modals are now handled by React components.
    // This file primarily handles map selection, global controls, and context menu for non-React parts.
    ...
    // Only admin can export/import map (buttons are conditionally rendered in PHP)
    ...
    // Only admin can create/rename/delete maps (buttons are conditionally rendered in PHP)
    if (window.userRole === 'admin') {
    ...
    // Only admin can add edges (button is conditionally rendered in PHP)
    if (window.userRole === 'admin') {
    ...
    // Scan Network button (conditionally rendered in PHP)
    if (window.userRole === 'admin') {
        els.scanNetworkBtn.addEventListener('click', () => openModal('scanModal'));
        els.closeScanModal.addEventListener('click', () => closeModal('scanModal')); // This modal is removed from PHP, but keeping listener for completeness if it were to be re-added via JS
    }

    // Map Settings Modal Logic (Admin only, button is conditionally rendered in PHP)
    if (window.userRole === 'admin') {
        els.mapSettingsBtn.addEventListener('click', () => {
            ...
            if (currentMap) {
                ...
                openModal('mapSettingsModal'); // This modal is removed from PHP, but keeping listener for completeness if it were to be re-added via JS
            }
        });
        els.cancelMapSettingsBtn.addEventListener('click', () => closeModal('mapSettingsModal')); // This modal is removed from PHP
        ...
        els.mapSettingsForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                closeModal('mapSettingsModal'); // This modal is removed from PHP
        ...
        els.mapBgUpload.addEventListener('change', async (e) => {
            const file = e.target.files[0];
            if (!file) return;
            const loader = document.getElementById('mapBgUploadLoader'); // This loader is removed from PHP
    ...
    // Place Device Modal Logic (Admin only, button is conditionally rendered in PHP)
    if (window.userRole === 'admin') {
        els.placeDeviceBtn.addEventListener('click', async () => {
            openModal('placeDeviceModal'); // This modal is removed from PHP
            els.placeDeviceLoader.classList.remove('hidden'); // This loader is removed from PHP
            els.placeDeviceList.innerHTML = ''; // This list is removed from PHP
            try {
        ...
        els.closePlaceDeviceModal.addEventListener('click', () => closeModal('placeDeviceModal')); // This modal is removed from PHP
        els.placeDeviceList.addEventListener('click', async (e) => { // This list is removed from PHP
    ...
    (async () => {
        ...
        if (initialMapId) {
            ...
            if (deviceToEdit && state.nodes.get(deviceToEdit)) {
                // MapApp.ui.openDeviceModal(deviceToEdit); // Removed, now handled by React
