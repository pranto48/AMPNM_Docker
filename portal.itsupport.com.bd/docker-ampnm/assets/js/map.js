        ...
        const options = { 
            ...
            manipulation: { 
                ...
                addEdge: async (edgeData, callback) => {
                    console.log("addEdge callback triggered. edgeData:", edgeData);
                    if (window.userRole !== 'admin') { // Corrected condition
                    ...
                    if (!edgeData.from || !edgeData.to) {
                        window.notyf.error('Invalid connection: missing source or target.');
                        callback(null);
                        return;
                    }

                    try {
                        const defaultConnectionType = 'cat5'; 
                        ...
                        console.log("API create_edge response:", newEdge);

                        window.notyf.success('Connection saved. Please select connection type.');
                        
                        // Update edgeData with the ID and default type from the database
                        ...
                        // Removed MapApp.state.network.disableEditMode() from here
                        // It should automatically exit after callback(edgeData) is called.
