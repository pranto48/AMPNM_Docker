...
MapApp.ui = {
    ...
    cacheElements: () => {
        ...
        MapApp.ui.els = {
            ...
            // NEW Add Device Modal elements
            addDeviceModal: getEl('addDeviceModal'),
            addDeviceForm: getEl('addDeviceForm'),
            addDeviceMapId: getEl('addDeviceMapId'),
            addDeviceName: getEl('addDeviceName'),
            addDeviceIp: getEl('addDeviceIp'),
            addDeviceType: getEl('addDeviceType'),
    ...
    openAddDeviceModal: () => {
        if (window.userRole !== 'admin') {
            window.notyf.error('You do not have permission to add devices.');
            return;
        }
        if (!MapApp.state.currentMapId) {
            window.notyf.error('Please select a map first to add devices.');
            return;
        }
        // Pre-fill map ID and clear form fields
        if (MapApp.ui.els.addDeviceMapId) MapApp.ui.els.addDeviceMapId.value = MapApp.state.currentMapId;
        if (MapApp.ui.els.addDeviceName) MapApp.ui.els.addDeviceName.value = '';
        if (MapApp.ui.els.addDeviceIp) MapApp.ui.els.addDeviceIp.value = '';
        if (MapApp.ui.els.addDeviceType) MapApp.ui.els.addDeviceType.value = 'server'; // Default type
...
function initMap() {
    ...
    if (window.userRole === 'admin' && els.edgeForm) {
        els.edgeForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Update the edge in the dataset to trigger re-render
    ...
    // NEW: Add Device Form Submission
    if (window.userRole === 'admin' && els.addDeviceForm) {
        els.addDeviceForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const name = els.addDeviceName.value.trim();
            const ip = els.addDeviceIp.value.trim();
            const type = els.addDeviceType.value;
            const map_id = els.addDeviceMapId.value;

            if (!name) {
                window.notyf.error('Device name cannot be empty.');
                return;
            }
            if (!map_id) {
                window.notyf.error('Map ID is missing. Please select a map.');
                return;
            }

            const submitButton = els.addDeviceForm.querySelector('button[type="submit"]');
            if (submitButton) {
            ...
            try {
                ...
                // Add the new device to the map's nodes dataset
                const baseNode = {
                    id: createdDevice.id,
                ...
                window.notyf.success(`Device "${name}" added successfully!`);
            } catch (error) {
                ...
                if (submitButton) {
                    ...
                    submitButton.innerHTML = '<i class="fas fa-plus mr-2"></i>Add Device';
                }
            }
        });
    }
    ...
    if (window.userRole === 'admin' && els.addDeviceBtn) {
        els.addDeviceBtn.addEventListener('click', () => MapApp.ui.openAddDeviceModal());
    } else if (els.addDeviceBtn) {
        els.addDeviceBtn.disabled = true;
    }
    ...
    (async () => {
        ...
        if (initialMapId) {
            ...
            // This is no longer needed as addDeviceBtn now opens a modal
        ...
        if (window.userRole === 'viewer') {
            ...
            if (els.deleteMapBtn) els.deleteMapBtn.disabled = true;
            if (els.placeDeviceBtn) els.placeDeviceBtn.disabled = true;
            if (els.addDeviceBtn) els.addDeviceBtn.disabled = true; // Disable the new modal button for viewers
